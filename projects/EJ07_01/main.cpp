...
void rotateLight(float angle) {
    ...
    // Rotamos la luz sobre el eje de la esfera. Para ello, movemos la luz al espacio de la esfera (poner la esfera como centro) y rotamos. En este caso, como la esfera esta en el (0,0,0), no hace falta mover la luz.

    const float xNew =  lightPos.x * c - lightPos.z * s; 
    const float zNew = lightPos.x * s + lightPos.z * c;
